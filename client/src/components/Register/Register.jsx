...
const Register = ({ loadUser, toggleSignIn }) => {
  ...
  const onConfirmationIdChange = (event) => {
    setRegisterState({
      ...registerState,
      registerConfirmationId: event.target.value
    });
  };
  ...
  const onConfirmationIdError = (show) => {
    if (show) {
      setRegisterState({
        ...
        confirmationIdErrorMessage: "Confirmation Id is required and must match the code you received via email"
      });
      registerConfirmationIdRef?.current?.classList.add("highlightClassInRegister");
    ...
    registerConfirmationIdRef?.current?.classList.remove("highlightClassInRegister");
  };
  ...
  const onRegisterStep1 = async () => {
    ...
    if (
      ...
      const response = await fetch(`/api/register-step-1`, {
        method: "post",
  ...
  const getProfileAndSignIn = async (data) => {
    try {
      const resp = await fetch(`/api/profile/${data.userId}`, {
        method: "GET",
        headers: {
      ...
      <main className="registerMain">
        <div className="registerMeasure">
          <fieldset
            ...
            {registerStepNum === 1 ? (
              ...
              <div
                ...
                />
                {showConfirmationIdError && (
                  <p className="registerErrorDisplay registerFieldsError">{confirmationIdErrorMessage}</p>
                )}
                <input
          ...
          <div className="belowRegisterButtonDiv">
            {registerStepNum === 2 && (
              <span className="belowRegisterButtonSpan1">Or, if you already have an account, go back to</span>
            )}
            <span
              ref={signinLinkRef}
