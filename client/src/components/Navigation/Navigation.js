...
const Navigation = ({ onRouteChange, isSignedIn, toggleModal, profilePhotoUrl, showLightning }) => {
  if (isSignedIn) {
    return (
      <nav className="nav">
        <Logo showLightning={showLightning} />
        <div className="gridCol2">
          <ProfileIcon profilePhotoUrl={profilePhotoUrl} onRouteChange={onRouteChange} toggleModal={toggleModal} />
        </div>
      </nav>
    );
  } else {
